...
#             : Time_part = time_build + OperatingTime_Per_Part = time_production_perPart (with respect to weight_product)
    ...
    def updateVarianceForPlotting(self, variableChanged, value):
        successValue = value
        ...
        elif(str(variableChanged).capitalize().find("Kw_price")):
            self._cost_KW = value
        else:
            print("stop 3akk bro, variable in Range_To_loop_on has a spelling mistake" + 
                  "Options are PartsProduced, processingPerPart, material_raw_product, Kw_price")
            value = -1
        return value
